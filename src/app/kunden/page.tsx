...
export default function Dashboard() {
  const allPages = ['dashboard', 'kunden', 'rechnungen', 'finanzen', 'einstellungen'];
  const allSVGS = {
    'dashboard': <svg className="ml-[22px] min-w-[28px] min-h-[28px] size-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" /></svg>,
    'kunden': <svg className="ml-[22px] min-w-[28px] min-h-[28px] size-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12 12.75a5.995 5.995 0 0 0-5.058 2.772m0 0a3 3 0 0 0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197a5.971 5.971 0 0 0-.94 3.197M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z" /></svg>,
    'rechnungen': <svg className="ml-[22px] min-w-[28px] min-h-[28px] size-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg>,
  ...
  useEffect(() => {
    ...
    setPage(window.location.href.split('/').filter(Boolean).pop());
  }, []);
    ...
    <div className="flex bg-[#f4f3ef]">
        <div className="sidebar flex flex-col h-[100vh] overflow-hidden" style={{ background: 'linear-gradient(#66615b,#000 80%)' }}>
            ...
            {allPages.map((item, index) => {
              return (
                <a href={`/${item}`} className={`flex items-center gap-[12px] pt-[20px] pb-[20px] cursor-pointer ${item === page ? 'text-[#00a2a2]' : 'text-[rgba(255,255,255,0.6)] hover:text-[rgba(255,255,255,1)]'} transition-colors duration-300 ease-in-out`} key={index}>
                  {allSVGS[item]}
                  <span className='text whitespace-nowrap overflow-hidden text-[14px] font-bold'>{item.toUpperCase()}</span>
                </a>
              )
            })}
        </div>
        <div className="flex flex-col h-[100vh] content">
          <div className="flex items-center border-b border-gray-300 w-full h-[63px] pl-[31px] pr-[31px]">
            ...
            <h1 className='text-[rgba(0,0,0,1)] capitalize'>{page}</h1>
          ...
          <div className="flex flex-col p-[64px]">
            <div className="flex justify-between items-center mb-[32px]">
              <input type="text" placeholder='Suchen' className='text-[#00a2a2] p-[8px] w-[50%] bg-[rgba(0,162,162,.1)] rounded-[4px] outline-none' />
              <button className='bg-[#00a2a2] text-white rounded-[4px] px-[10px] py-[5px] ml-[10px] cursor-pointer'>Kunden hinzufügen</button>
            </div>
            <div className="flex relative">
              <button className={`text-[#000] p-[8px] cursor-pointer`} style={{borderBottom: `${productType ? 'none' : '3px solid #00a2a2'}`, paddingBottom: `${productType ? '8px' : '5px'}`}} onClick={() => {setProductType(false)}}>Alle Kunden</button>
              <button className={`text-[#000] p-[8px] ml-[8px] cursor-pointer`} style={{borderBottom: `${productType ? '3px solid #00a2a2' : 'none'}`, paddingBottom: `${productType ? '5px' : '8px'}`}} onClick={() => {setProductType(true)}}>Beliebte Kunden</button>
              <div className='absolute bottom-0 right-0 w-full h-[3px] rounded-full bg-[rgba(0,0,0,0.2)]'></div>
            </div>
            <div className="overflow-x-auto mt-[32px]">
              <table className="min-w-full rounded-[4px] shadow-sm">
                <thead className="text-left text-[#666] text-sm">
